...
class Settings(BaseModel):
    ...
    default_daily_requests_limit: int = Field(
        1000, description="Default daily requests limit per API key"
    )
    ...
    rate_limit_requests: int = Field(
        100, description="Number of requests allowed in the rate limit window"
    )
